...
def segment(img, s_thresh=(170, 255), sx_thresh=(20, 100)):
    img = np.copy(img)
...
def warp(img):
    ...
    # Four source coords.
    # TODO: consider tuning points better.
    src = np.float32(
        [[265, 674],
         [1039, 674],
         [552, 478],
...
def find_lane_pixels(binary_warped):
    ...
    # TODO: consider feeding directly a grayscale img
    gray_warped = cv2.cvtColor(binary_warped, cv2.COLOR_BGR2GRAY)
    histogram = np.sum(gray_warped, axis=0)
    # plt.plot(histogram)
    # plt.waitforbuttonpress()
...
# Calibrate camera.
imgpoints, objpoints = calibrate_cam(images, nx, ny)
...
segmented = segment(undistorted)

# Warp image.
warped_img = warp(segmented)

# Segment the two lanes
lanes_img = fit_polynomial(warped_img)

# Unwarp image.
unwarped_img = unwarp(lanes_img)

# Merge the two images
output_img = weighted_img(image, unwarped_img, α=0.8, β=1., γ=0.)
